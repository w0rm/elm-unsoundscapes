...
**Note: The original `StartApp` had to be modified to use `foldl'`
from `Apanatshka/elm-signal-extra` in order to use initial signal value!***
...
start config =
    let
        ...
        inputs =
            Signal.mergeMany (List.map (Signal.map singleton) config.inputs ++ [messages.signal])

        -- init : List action -> (model, Effects action)
        init actions =
            update actions config.init

        -- effectsAndModel : Signal (model, Effects action)
        effectsAndModel =
            foldp' update init inputs
